...
Явка на вайп использует текущий состав привата с ролью Rust (боты исключены).
Ближайший вайп открывается первым; список подтверждений, отказов с причинами и неответивших разбит на страницы.
Ответы закрываются в начале вайпа. Через 5 часов cron выдаёт варн неответившим из первоначального состава, если они всё ещё в клане; отметка «Зашёл» освобождает от этой автоматической проверки.
Отказ с причиной не вызывает автоматического варна. Нарушенное обещание прийти наказывается при отметке модератора «Не зашёл».
В записи явки сохраняется факт выдачи варна, чтобы смена отметки не выдавала новый. Старые завершённые вайпы без зафиксированного состава не обрабатываются задним числом.
Для получения состава включите Server Members Intent у приложения Discord.

- панель заявок `/setup-recruitment`;
- открытие и закрытие набора из приватной админ-панели с блокировкой всех незавершённых шагов анкеты;
- двухшаговая анкета: основные данные, затем до пяти Steam-аккаунтов в отдельных полях;
...
- после принятия — ссылка и кнопка входа на .int Private, команда `/claim` для автоматической выдачи ролей и ника;
- принятый тикет автоматически закрывается через час (проверка выполняется каждые 10 минут);
- `/claim` выдаёт новичку временную роль на 7 суток; бот снимает её автоматически;
- в админ-панели есть прямая кнопка на статистику клана .int на Mirage Rust;
- Vipe Info поддерживает ссылку/скриншот карты, заметный квадрат спота для строительства (например, B6) и изменяемые ответы «Буду / Опоздаю / Не смогу»; для отсутствия обязательно указывается причина, а списки игроков видны в карточке;
- через 5 часов после вайпа Staff отмечает явку; отметка «Не зашёл» автоматически выдаёт варн;
...
- при отказе по возрасту Staff может принять кандидата одной кнопкой как исключение;
